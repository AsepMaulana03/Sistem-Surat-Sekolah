...
<div>
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Approval Surat (Pihak 1)</h1>
            <p class="text-sm text-gray-500 mt-0.5">Surat yang menunggu persetujuan Anda sebagai Pihak 1</p>
        </div>
        @php 
            $totalPending = \App\Models\Letter::where('status','menunggu_persetujuan_pihak1')
                ->where('pihak1_id', Auth::id())->count(); 
        @endphp
        @if($totalPending > 0)
    ...
    <!-- Filter & Search -->
    <form method="GET" action="{{ route('guru.approval') }}" class="mb-6">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="px-5 py-4">
                        <div class="flex items-center justify-center gap-2">
                            ...
                            <!-- Setujui -->
                            <form action="{{ route('guru.approve', $letter) }}" method="POST" style="display:inline">
                                @csrf
                                <button type="submit"
                                    onclick="return confirm('Setujui surat ini untuk dilanjutkan ke Kepala Sekolah?')"
                ...
                <tr>
                    <td colspan="7" class="px-5 py-16 text-center">
                        <div class="flex flex-col items-center gap-3">
                            ...
                            <p class="text-gray-500 font-semibold">Tidak ada surat yang menunggu persetujuan Anda</p>
                        </div>
                    </td>
                </tr>
                @endforelse
...
<script>
    function openRejectModal(letterId) {
        document.getElementById('reject-form').action = '/guru/approval/' + letterId + '/reject';
