...
<div class="grid grid-cols-1 lg:grid-cols-5 gap-8">
    ...
    <div class="lg:col-span-3 bg-white rounded-2xl border border-gray-100 shadow-sm p-6 flex flex-col">
        ...
        <div class="flex-1 bg-gray-100 rounded-xl overflow-hidden border border-gray-200 flex items-center justify-center min-h-[700px]">
            @php
            ...
            @else
                <iframe src="{{ Storage::url($file_path) }}#toolbar=0&view=FitH" frameborder="0" class="w-full h-full min-h-[700px]"></iframe>
            @endif
        </div>
    </div>
        ...
        <form action="{{ route('incoming-letters.store') }}" method="POST">
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="tujuan" value="{{ old('tujuan', $ocr_data['tujuan'] ?? '') }}" required class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 text-gray-900 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors">
